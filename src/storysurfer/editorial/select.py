...
from storysurfer.domain import Comment, SelectionCandidate, SelectionResult, ThreadSnapshot
from storysurfer.editorial.text import content_words, plain_text, similarity, word_count, words
from storysurfer.errors import SelectionError
...
def select_thread(snapshot: ThreadSnapshot, config: SelectionConfig) -> SelectionResult:
    ...
    scored = [
        _score_candidate(candidate, post_text, config, max_reddit_score) for candidate in candidates
    ]
    ...
    if word_count(snapshot.submission.body) > post_budget:
        warnings.append(
            "The complete post exceeds its duration allocation; the final narration may "
            "run longer than the target unless a reviewer edits it."
        )
