"""Command-line and local web entry points."""
...
from storysurfer.config import AppConfig, load_config
from storysurfer.domain import ThreadSnapshot
from storysurfer.errors import ConfigurationError, StorySurferError
from storysurfer.media import media_for_profile, probe_media
...
def build_parser() -> argparse.ArgumentParser:
    ...
    web_parser = subparsers.add_parser("web", help="launch the local browser interface")
    web_parser.add_argument("--host", default="127.0.0.1")
    web_parser.add_argument("--port", type=int, default=7860)
    web_parser.add_argument(
        "--no-browser", action="store_true", help="do not open a browser window"
    )
    _add_config_argument(web_parser)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "web":
            return _web(config, arguments.host, arguments.port, not arguments.no_browser)
        ...
        if arguments.command == "ingest":
            ...
            print(f"Next: uv run storysurfer select {run_id}")
            return 0
        ...
        if arguments.command == "script":
            ...
            print(f"Next: uv run storysurfer narrate {arguments.run_id}")
            return 0
        if arguments.command == "narrate":
            ...
            print(f"Next: uv run storysurfer caption {arguments.run_id}")
            return 0
        if arguments.command == "caption":
            ...
            print(
                "Next: uv run storysurfer preview "
...
def _web(config: AppConfig, host: str, port: int, inbrowser: bool) -> int:
    if not 1 <= port <= 65_535:
        raise ConfigurationError("Web port must be from 1 to 65535.")
    if host not in {"127.0.0.1", "localhost", "::1"}:
        print(
            "Warning: non-loopback binding exposes this development UI to the network; "
            "it is not production-ready.",
            file=sys.stderr,
        )
    from storysurfer.gradio_ui import create_gradio_app

    create_gradio_app(config).launch(host, port, inbrowser)
    return 0
